 ...
 * (isLocked = false) or the calling thread is the thread that locked the Lock
 * instance, the while loop will not execute, and the thread calling lock() will
 ...
 * lock, even if the lock has been locked multiple times. We don't want the lock
 * to be unlocked until the thread that locked it, has executed the same amount
